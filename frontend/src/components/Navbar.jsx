...
const NavItems = ({ route, url, classProps }) => {
	return (
		<a
			href={url}
			target="_blank"
		...
		<nav className="w-full flex md:justify-center justify-between items-center p-4">
			...
			<ul className="text-white md:flex hidden list-none flex-row justify-between items-center flex-initial">
				{[
					{
						route: "Coins",
						url: "https://iseriouslyhate.tech/cryptomart/cryptos",
					},
					{ route: "News", url: "https://iseriouslyhate.tech/cryptomart/news" },
					{
						route: "Faucet",
						url: "https://iseriouslyhate.tech/cryptomart/news",
					},
				].map((item, i) => (
					<NavItems key={i} {...item} />
				))}
				...
				{toggleMenu && (
					<ul className="z-10 fixed top-0 -right-2 p-3 w-auto h-auto shadow-2xl md:hidden list-none flex flex-col justify-start items-end rounded-md blue-glassmorphism text-white animate-slide-in">
						...
						{[
							...
							<NavItems key={i} {...item} classProps="my-2 text-lg" />
						))}
					</ul>
				)}
			</div>
